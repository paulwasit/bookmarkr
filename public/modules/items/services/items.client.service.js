...
.service('items', ['$rootScope', 'pwArrayToggle', function ($rootScope, pwArrayToggle) {
...
// -------------------- ITEMS ------------------------------------------------------ //
	...
	this.getItems = function (newItems) {
		items = newItems;
		tags = this.getUniqueTags (items, 'genre');
	};
...
// -------------------- EDIT MODE -------------------------------------------------- //
	...
	this.toggleId = function (value) {
		...
		if (this.isEditMode() === false) {
			...
			edit.query = this.getNewBooleanState (items, edit.ids, '_id', ['archived', 'inTrash', 'favorite']);
			edit.tags = this.getTagsBooleanState(items, edit.ids, '_id', 'genre');
	...
	this.toggleTag = function (value) {
		...
		items = this.updateItems (items, edit.ids, '_id', 'genre', edit.tags);
...
// -------------------- UPDATE ------------------------------------------------------- //
	...
	this.updateItems = function (items, selectedIDs, idFieldName, itemFieldName, newValue) {
		...
		for (var i=0, len=items.length; i<len;i++) {
			if (selectedIDs.indexOf (items[i][idFieldName]) !==-1) {
				if (itemFieldName === 'genre') {
					
					var itemTags = items[i].genre;
					var newValNames = [];
	...
	this.clientUpdate = function (field) {
		...
		items = this.updateItems (items, edit.ids, '_id', field, edit.query[field]);
		edit.ids = [];
		$rootScope.$broadcast('itemsUpdate', items);
	};
